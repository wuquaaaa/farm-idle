...
const TABS = ['farm', 'build', 'tech', 'market', 'worker'] as const;
...
export function App() {
  ...
  useEffect(() => {
    ...
    engine.onStateChange(() => {
      setState({ ...engine.getState() as GameState });
    });
  ...
  if (!state) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-stone-50">
        <div className="text-stone-400 text-lg animate-pulse">加载中...</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-stone-50 flex flex-col">
      ...
      <div className="flex-1 p-4 max-w-md mx-auto w-full pb-24">
        {tab === 'farm' && <FarmView state={state} dispatch={dispatch} />}
